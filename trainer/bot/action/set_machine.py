...
from trainer.bot.replykey_board import button_start, muscle_group, button_spesific_muscule, button_list_weight, \
    button_number, button_menu
...
from trainer.bot.text import already_not_registered, muscle_selection_text, choose_muscle_text, one_set_trening_text, \
    ...
    tree_set_repetition_text, add_machine_complite_text, add_machine_text
...
@router.message(F.text == f'{add_machine_text}')
...
@router.message(GetMachine.group_muscles)
...
@router.message(GetMachine.title_machin)
async def set_age(message, state: FSMContext):
...
@router.message(GetMachine.set_1_weight)
async def set_age(message, state: FSMContext):
...
@router.message(GetMachine.set_2_weight)
async def set_age(message, state: FSMContext):
...
@router.message(GetMachine.set_3_weight)
async def set_age(message, state: FSMContext):
...
@router.message(GetMachine.iter_1)
async def set_age(message, state: FSMContext):
...
@router.message(GetMachine.iter_2)
async def set_age(message, state: FSMContext):
...
@router.message(GetMachine.iter_3)
async def set_age(message, state: FSMContext):
